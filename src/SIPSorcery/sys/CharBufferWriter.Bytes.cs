...
internal static partial class CharBufferWriter
{
    ...
    {
        public IBufferWriter<char> Append(byte[] bytes, char? separator = null)
        {
            if (bytes is { Length: > 0 })
            {
                writer.Append(bytes.AsSpan(), separator);
            }

            return writer;
        }

        public IBufferWriter<char> Append(scoped ReadOnlySpan<byte> bytes, char? separator = null, bool lowercase = false)
        {
            if (bytes.IsEmpty)
            {
                return writer;
