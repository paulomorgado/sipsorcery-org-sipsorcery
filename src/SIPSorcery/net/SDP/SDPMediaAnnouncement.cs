...
namespace SIPSorcery.Net
{
    ...
    public class SDPMediaAnnouncement
    {
        ...
        /// <summary>
        /// If being used in a bundle this the ID for the writer.
        ...
        /// <summary>
        /// The stream status of this media writer.
        /// </summary>
        ...
        public void AddCryptoLine(string crypto)
            => AddCryptoLine(crypto.AsSpan());

        public void AddCryptoLine(ReadOnlySpan<char> crypto)
        {
            if (!SDPSecurityDescription.TryParse(crypto, out var securityDescription))
            {
                throw new FormatException($"cryptoLine '{crypto.ToString()}' is not recognized as a valid SDP Security Description ");
            }

            this.SecurityDescriptions.Add(securityDescription);
        }
