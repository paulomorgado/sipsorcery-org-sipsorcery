...
namespace SIPSorcery.Net
{
    ...
    public class SDPMediaAnnouncement
    {
        ...
        public void ParseMediaFormats(string formatList)
        {
            ...
            foreach (var range in span.SplitAny(SipSorcerySearchValues.WhiteSpaceChars))
            {
        ...
        public void WriteString(IBufferWriter<char> writer)
        {
            ...
            writer.Append(m_CRLF);

            if (!string.IsNullOrWhiteSpace(MediaDescription))
            {
                writer.Append("i=").Append(MediaDescription).Append(m_CRLF);
            }
            ...
            if (TIASBandwidth > 0)
            {
                writer.Append(TIAS_BANDWIDTH_ATTRIBUE_PREFIX).Append(TIASBandwidth).Append(m_CRLF);
            }

            foreach (var bandwidthAttribute in BandwidthAttributes)
            {
                writer.Append("b=").Append(bandwidthAttribute).Append(m_CRLF);
            }

            if (!string.IsNullOrWhiteSpace(IceUfrag))
            {
                writer.Append("a=" + SDP.ICE_UFRAG_ATTRIBUTE_PREFIX + ":").Append(IceUfrag).Append(m_CRLF);
            }

            if (!string.IsNullOrWhiteSpace(IcePwd))
            {
                writer.Append("a=" + SDP.ICE_PWD_ATTRIBUTE_PREFIX + ":").Append(IcePwd).Append(m_CRLF);
            }

            if (!string.IsNullOrWhiteSpace(DtlsFingerprint))
            {
                writer.Append("a=" + SDP.DTLS_FINGERPRINT_ATTRIBUTE_PREFIX + ":").Append(DtlsFingerprint).Append(m_CRLF);
            }

            if (IceRole is { } iceRole)
            {
                writer.Append("a=" + SDP.ICE_SETUP_ATTRIBUTE_PREFIX + ":").Append(iceRole.ToStringFast()).Append(m_CRLF);
            }

            if (IceCandidates is { Count: > 0 })
            {
                foreach (var candidate in IceCandidates)
                {
                    writer.Append("a=" + SDP.ICE_CANDIDATE_ATTRIBUTE_PREFIX + ":").Append(candidate.ToString()).Append(m_CRLF);
                }
            }

            if (IceOptions is { })
            {
                writer.Append("a=" + SDP.ICE_OPTIONS + ":").Append(IceOptions).Append(m_CRLF);
            }

            if (IceEndOfCandidates)
            {
                writer.Append("a=" + SDP.END_ICE_CANDIDATES_ATTRIBUTE).Append(m_CRLF);
            }

            if (!string.IsNullOrWhiteSpace(MediaID))
            {
                writer.Append("a=" + SDP.MEDIA_ID_ATTRIBUTE_PREFIX + ":").Append(MediaID).Append(m_CRLF);
            }
            ...
            {
                writer.Append(MEDIA_EXTENSION_MAP_ATTRIBUE_PREFIX).Append(headerExtension.Value.Id).Append(' ')
            ...
            foreach (var extra in ExtraMediaAttributes)
            {
                ...
                {
                    writer.Append(extra).Append(m_CRLF);
                }
            }
            ...
            {
                writer.Append(MediaStreamStatusType.GetAttributeForMediaStreamStatus(MediaStreamStatus.Value)).Append(m_CRLF);
            }

            if (SsrcGroupID is { } && SsrcAttributes.Count > 0)
            {
                writer.Append(MEDIA_FORMAT_SSRC_GROUP_ATTRIBUE_PREFIX).Append(SsrcGroupID);
                foreach (var ssrcAttr in SsrcAttributes)
                {
                    writer.Append(' ').Append(ssrcAttr.SSRC);
                }
                writer.Append(m_CRLF);
            }
            ...
            if (SsrcAttributes.Count > 0)
            {
                foreach (var ssrcAttr in SsrcAttributes)
                {
                    ...
                    {
                        writer.Append(MEDIA_FORMAT_SSRC_ATTRIBUE_PREFIX).Append(ssrcAttr.SSRC).Append(' ')
                            .Append(SDPSsrcAttribute.MEDIA_CNAME_ATTRIBUE_PREFIX).Append(':').Append(ssrcAttr.Cname).Append(m_CRLF);
                    }
                    else
                    {
                        writer.Append(MEDIA_FORMAT_SSRC_ATTRIBUE_PREFIX).Append(ssrcAttr.SSRC).Append(m_CRLF);
                    }
                }
            }
            ...
            if (SctpMap is { })
            {
                writer.Append(MEDIA_FORMAT_SCTP_MAP_ATTRIBUE_PREFIX).Append(SctpMap).Append(m_CRLF);
            }
            else
            {
                if (SctpPort is { })
                {
                    writer.Append(MEDIA_FORMAT_SCTP_PORT_ATTRIBUE_PREFIX).Append(SctpPort).Append(m_CRLF);
                }

                if (MaxMessageSize != 0)
                {
                    writer.Append(MEDIA_FORMAT_MAX_MESSAGE_SIZE_ATTRIBUE_PREFIX).Append(MaxMessageSize).Append(m_CRLF);
                }
            }
        }
        ...
        public void WriteFormatListAttributesString(IBufferWriter<char> writer)
        {
            if (Media == SDPMediaTypesEnum.application)
            {
                foreach (var appFormat in ApplicationMediaFormats)
                {
                    ...
                    {
                        writer.Append(MEDIA_FORMAT_ATTRIBUTE_PREFIX).Append(appFormat.Key).Append(' ')
                            .Append(appFormat.Value.Rtpmap).Append(m_CRLF);
                    }

                    if (appFormat.Value.Fmtp != null)
                    {
                        writer.Append(MEDIA_FORMAT_PARAMETERS_ATTRIBUE_PREFIX).Append(appFormat.Key).Append(' ')
            ...
            else if (Media == SDPMediaTypesEnum.message)
            {
                ...
                {
                    writer.Append(MEDIA_FORMAT_PATH_ACCEPT_TYPES_PREFIX);
                ...
                if (mediaFormat.Endpoint != null)
                {
                    ...
                    writer.Append(MEDIA_FORMAT_PATH_MSRP_PREFIX).Append("//").Append(Connection.ConnectionAddress).Append(':')
                ...
                if (MediaFormats != null)
                {
                    foreach (var entry in MediaFormats)
                    {
                        ...
                        if (mediaFormat.Rtpmap == null)
                        {
                            ...
                            writer.Append(MEDIA_FORMAT_ATTRIBUTE_PREFIX).Append(mediaFormat.ID).Append(' ')
                        ...
                        if (mediaFormat.Kind == SDPMediaTypesEnum.video)
                        {
                            ...
                            {
                                writer.Append(MEDIA_FORMAT_FEEDBACK_PREFIX).Append(mediaFormat.ID).Append(' ')
                        ...
                        {
                            writer.Append(MEDIA_FORMAT_PARAMETERS_ATTRIBUE_PREFIX).Append(mediaFormat.ID).Append(' ')
