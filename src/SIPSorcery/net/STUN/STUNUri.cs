...
public sealed class STUNUri : IEquatable<STUNUri>
{
    ...
    public static bool TryParse(ReadOnlySpan<char> uriSpan, [NotNullWhen(true)] out STUNUri? uri)
    {
        ...
        if (lastColonPos != -1)
        {
            ...
            else
            {
                ...
                if (hostSpan.IsEmpty || hostSpan.IndexOfAny(SipSorcerySearchValues.InvalidHostNameChars) >= 0)
                {
                    return false;
                }
        ...
        {
            if (uriSpan.IsEmpty || uriSpan.IndexOfAny(SipSorcerySearchValues.InvalidHostNameChars) >= 0)
            {
                return false;
            }
    ...
    public override string ToString()
    {
        ...
        builder.Append(Scheme.ToStringFast());
        ...
        {
            builder.Append('?');
            builder.Append(SCHEME_TRANSPORT_SEPARATOR);
            builder.Append(Transport.ToStringFast());
        }
